...
mainApp.controller('ComparisonCtrl', function ($scope, $http,$window) {
    ...
    $scope.fencingData={
        ...
        , Show_motions:true            // show motions or tactic nodes
        , Sort_phrases:false
        , nodeBias:{
            "S" :{w1:0.0,w2:0.0},
            "BB":{w1:0.2,w2:0.2},
    ...
    function generatePhrases(series,arrMotion){
        ...
        var state1="";                   // state of player 1
        var state2="";                   // state of player 2
        index=0;
        series.forEach(function(d){
            if(d.result){
                ...
                //    else if(currentState=="BB" && state1!="B" && state2!="B") arrStates.push("FF");
                //    if(index==20) console.log(currentState,state1,state2,arrStates);
                ...
                // remove BB FF FB(BF)
                if(arrStates.length>4 && arrStates[2]=="FF") arrStates.splice(2,1);
                ...
                function mapState(foot){
                    if(foot=="fs") return "F"
                    else if(foot=="bs") return "B"
                    else if(foot=="as") return "A";
                    else return "";
                }
                var s1=mapState(d.foot1);
                var s2=mapState(d.foot2);
                var newState1=s1?s1:state1;
                var newState2=s2?s2:state2;

                //if(index==36) console.log(state1,state2,newState1,newState2,currentState);

                if(state1=="A" && newState1!="A"){
                    currentState="FB";
                    arrStates.push(currentState);
                }
        ...
        var bOutputSequence=false;
        if(bOutputSequence){
            var sequence=[];

            phrases.forEach(function(d){
                sequence.push(d.states);
            })
            console.log(JSON.stringify(sequence))
        }
        // print the motion sequence: used for debug
    ...
    function readDataV2(){
        var arrFiles=[
            "../data/men_semifinal_1_v2.csv"
            ,"../data/men_semifinal_2_v2.csv"
            ,"../data/men_final_v2.csv"
        ]
        var arrSeries=[];
        // read the file of index
        function readFile(index){
            arrSeries.push([]);
            d3.csv(arrFiles[index], function(d) {
                var frame=parseFrame(d.time);
                arrSeries[index].push({
                    frame:frame
                    ,foot1:d.foot1
                    ,foot2:d.foot2
                    ,hand1:d.hand1
        ...
        function preprocessData(){
            ...
            var arrMotions=[];
            arrSeries.forEach(function(d){
                arrMotions.push(generateMotionData(d))
            })
            // 2.generate bout data
            var arrPhrases=[];
            arrSeries.forEach(function(d,i){
                arrPhrases.push(generatePhrases(d,arrMotions[i]))
            })

            // 3.generate tactic flow data
            var arrFlows=[];
            arrPhrases.forEach(function(d){
                arrFlows.push(generateFlow_2(d))
            })
            var flow=combineFlows(arrFlows[0][0],arrFlows[1][0]);
            flow=combineFlows(flow,arrFlows[2][0]);

            // merge phrases
            var phrases=[];
            arrPhrases.forEach(function(d,i){
            ...
            var motions_foot=[];
            var motions_blade=[];
            arrMotions.forEach(function(d){
                d[0].forEach(function(dd,i){
                    dd.phrase+=i*100;
                    motions_blade.push(dd);
                })
                d[1].forEach(function(dd,i){
                    dd.phrase+=i*100;
                    motions_foot.push(dd);
                })
            })


            $scope.fencingData.motion_hands=motions_blade;
            $scope.fencingData.motion=motions_foot;
            $scope.fencingData.series=arrSeries[0];
            $scope.fencingData.phrases=phrases;
            $scope.fencingData.phrases_sorted=phrases_sorted;
            $scope.fencingData.flow=flow;
            $scope.fencingData.flow_player1=arrFlows[0][1];
            $scope.fencingData.flow_player2=arrFlows[0][2];
            $scope.fencingData.flow_groups=[arrFlows[0][0],arrFlows[1][0],arrFlows[2][0]];
